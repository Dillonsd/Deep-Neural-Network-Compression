...
# Define loaders
test_loader = gtsrb.get_test_data_loader(batch_size=128, shuffle=True)
...
# Test network prediction latency
import time
start = time.time()
for images, labels in test_loader:
  images = images.to(device)
...
print('Latency: {}ms'.format((end - start) * 1000 / len(test_loader)))
...
for b in net.buffers():
  buffers += b.numel() * b.element_size()
print('Model size: {}MB'.format((params + buffers) / (1024**2)))
...
with torch.no_grad():
  for i, (images, labels) in enumerate(test_loader):
    ...
    _, predicted = torch.max(outputs.data, 1)
    for j in range(5):
      ax[j].imshow(ToPILImage()(images[j].cpu()))
